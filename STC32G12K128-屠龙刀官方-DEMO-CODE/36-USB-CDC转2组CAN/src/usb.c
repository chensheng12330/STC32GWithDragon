...
BYTE xdata CDC1InBuffer[64];
BYTE xdata CDC1OutBuffer[64];
BOOL bCDC1InBusy;
BOOL bCDC1OutReady;

BYTE xdata CDC2InBuffer[64];
BYTE xdata CDC2OutBuffer[64];
BOOL bCDC2InBusy;
BOOL bCDC2OutReady;
...
//    BYTE cnt;
    ...
    if (csr & OUTOPRDY)
    {
        CDC1OutNumber = usb_read_fifo(FIFO4, CDC1OutBuffer);
        bCDC1OutReady = 1;
...
//    BYTE cnt;
    ...
    if (csr & OUTOPRDY)
    {
        CDC2OutNumber = usb_read_fifo(FIFO5, CDC2OutBuffer);
        bCDC2OutReady = 1;
...
void CDC1_IN(BYTE cnt)
{
    int timeout=200;
    EUSB = 0;
    
    usb_write_reg(INDEX, 4);
    while ((usb_read_reg(INCSR1) & INIPRDY) && timeout--);
    usb_bulk_intr_in(CDC1InBuffer, cnt, 4);
    Ep4InBusy = 1;
    
    EUSB = 1;
}

void CDC2_IN(BYTE cnt)
{
    int timeout=200;
    EUSB = 0;
    ...
    usb_write_reg(INDEX, 5);
    while ((usb_read_reg(INCSR1) & INIPRDY) && timeout--);
    usb_bulk_intr_in(CDC2InBuffer, cnt, 5);
    Ep4InBusy = 1;
...
void CDC1_SendData(BYTE *dat, int size)
{
    BYTE cnt;
    int addr;
    int timeout=200;
    ...
    do
    {
        cnt = size > 64 ? 64 : size;
        while ((usb_read_reg(INCSR1) & INIPRDY) && timeout--);
        usb_bulk_intr_in(&dat[addr], cnt, 4);
        addr += cnt;
        size -= cnt;
    } while (cnt >= 64);
...
void CDC2_SendData(BYTE *dat, int size)
{
    BYTE cnt;
    int addr;
    int timeout=200;
    ...
    do
    {
        cnt = size > 64 ? 64 : size;
        while ((usb_read_reg(INCSR1) & INIPRDY) && timeout--);
        usb_bulk_intr_in(&dat[addr], cnt, 5);
        addr += cnt;
        size -= cnt;
    } while (cnt >= 64);
