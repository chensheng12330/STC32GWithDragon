...
UNS8 sendEMCY(CO_Data* d, UNS16 errCode, UNS8 errRegister, const UNS8 errSpecific[5])
{
	...
	m.rtr = NOT_A_REQUEST;	
	m.len = 8;
	m.m_data[0] = errCode & 0xFF;        /* LSB */
	m.m_data[1] = (errCode >> 8) & 0xFF; /* MSB */
	m.m_data[2] = errRegister;

	if (errSpecific == NULL)	/* Manufacturer Specific Error Field */
		memset(&m.m_data[3], 0, 5);
	else
		memcpy(&m.m_data[3], errSpecific, 5);
...
void proceedEMCY(CO_Data* d, Message* m)
{
	...
	errCode = m->m_data[0] | ((UNS16)m->m_data[1] << 8);
	errReg = m->m_data[2];
	(*d->post_emcy)(d, nodeID, errCode, errReg, (const UNS8*)&m->m_data[3]);
}
