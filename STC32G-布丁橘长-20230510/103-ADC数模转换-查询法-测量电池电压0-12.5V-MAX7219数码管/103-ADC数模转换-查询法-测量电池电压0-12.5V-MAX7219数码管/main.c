// 	@布丁橘长 2023/03/16
// 	ADC示例：P0.0引脚测量电池电压，测量范围0~12.5V，MAX7219数码管模块显示
//  采用查询法，查询ADC_FLAG是否置1，来判断ADC是否转换完成
...
u8 volt_int1;									// 电压整数十位部分
u8 volt_int2;									// 电压整数个位部分
...
void main(void)
{
	...
	while (1)
	{
		...
		adc_result = ADC_RES*256 + ADC_RESL;				// 读取采样结果（右对齐方式）
		ADC_FLAG = 0;																// 清零ADC转换标志
		
		adc_volt = 5 * 2500 * adc_result / 4096 ;  	// 计算电池电压，单位：mV
		volt_float3 = (adc_volt)% 10;								// 小数点后第3位
		if(volt_float3 > 0) adc_volt += 10;				// 如果小数点后第3位大于0，小数点后第2位进一位	
		volt_int1 = (adc_volt /10000) % 10;					// 电池电压，整数十位部分
		volt_int2 = (adc_volt /1000) % 10;					// 电池电压，整数个位部分
...
{							
	...
	Write7219(4,(u16)(volt_int1)); 				// 左起第5位显示 电压整数十位部分
	Write7219(3,(u16)(volt_int2 | DP)); 	// 左起第6位显示 电压整数个位部分，并加上小数点
