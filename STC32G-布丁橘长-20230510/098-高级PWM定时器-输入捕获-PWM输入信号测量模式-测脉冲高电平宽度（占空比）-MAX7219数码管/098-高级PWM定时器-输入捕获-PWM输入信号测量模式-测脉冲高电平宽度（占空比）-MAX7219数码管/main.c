// 	@布丁橘长 2023/03/15
// 	PWM输入捕获示例：PWM输入信号测量模式，CC1捕获上升沿，CC2捕获下降沿，使能CC2捕获中断
//  PWM1P（P1.0）为信号测量引脚，PWM5（P2.0）输出占空比可调的PWM，板载P3.2 P3.3按键调节占空比，每次加减50us
//	方波由PWM5(P2.0)输出，周期：1000us，占空比50%（即高电平500us）
...
#define PWMB_PERIOD 1000			// 周期1毫秒，1KHz		
...
{
	if (PWMA_SR1 & 0X04)				// 当捕获中断标志置1
	{	
		PWMA_SR1 &= ~0X04;				// 清零捕获中断标志
...
//------------PWMB设置-PWM5(P2.0)输出一路PWM：1KHz，初始占空比：50%-------------------
