//		@布丁橘长 2022/11/17
//		定时器2-12T模式-16位自动重装载：定时10秒，每10秒翻转一次P2.0电平
...
void Timer2_Init(void);		//1毫秒@12.000MHz  定时器2初始化函数声明
...
void Timer2_Init(void)		//10秒@12.000MHz
{
	TM2PS = 0x98;			//设置定时器时钟预分频 ( 注意:并非所有系列都有此寄存器,详情请查看数据手册 )
