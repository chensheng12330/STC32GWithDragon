//		@布丁橘长 2022/11/17
//		定时器1-1T模式-模式0-16位自动重装载模式：定时1秒，每1秒翻转一次P2.0电平
...
void Timer1_Init(void);		//1毫秒@12.000MHz  定时器1初始化函数声明
...
void Timer1_Init(void)		//1秒@12.000MHz
{
	TM1PS = 0xB7;			//设置定时器时钟预分频 ( 注意:并非所有系列都有此寄存器,详情请查看数据手册 )
	AUXR |= 0x40;			//定时器时钟1T模式
	TMOD &= 0x0F;			//设置定时器模式-模式0-16位自动重装载模式
	TL1 = 0x3F;				//设置定时初始值
	TH1 = 0x01;				//设置定时初始值
	TF1 = 0;				//清除TF1标志
	TR1 = 1;				//定时器1开始计时
