//		@布丁橘长 2023/01/18
//		2片74HC595级联驱动4位共阳极数码管,显示2023
//		4位数码管模块-595级联驱动-共阳极
//		实验采用 段码数组+位码数组
//		A-DP接初级595输出Q0-Q7，COM1-Q3,COM2-Q2,COM3-Q1-COM4-Q0(数码管左边第一位接Q3，第二位Q2...)
//		74HC595引脚定义：DS(DIO)：P2.0 ST(SCK)：P2.1 SH(RCK)-P2.2
//		本实验采用准双向口模式
...
sbit DS = P2^0;				//串行输入DS(DIO)接P2.0
sbit ST = P2^1;				//锁存时钟接ST(RCK)P2.1
sbit SH = P2^2;				//串行输入时钟SH(SCK)接P2.2

void delayms(u16 ms);						//延时函数声明
void Write595(u8 dat);					//595写数据函数声明
void Output595();								//595输出函数声明
...
//共阳极位码数组-4位数码管
u8 WEI_Code[4] = {0x08,0x04,0x02,0x01};		//共阳极位码数组，COM1接595Q3，COM2接Q2...即数码管左边第一位接Q3...第四位接Q0
