//	@布丁橘长 2023/04/22
//  printf串口打印示例：USART1-模式1，主频22.1184MHz,波特率115200,8位数据方式，1位起始位，无校验位，1位停止位，
//  串口1使用默认引脚P3.0(RxD) P3.1(TxD)
//	按单片机P3.2按键，发送各种类型数据给电脑端，电脑端发送字符'1'-'8'，单片机P2口对应LED闪烁3次
...
char number2;									// 测试变量，打印次数	char 类型
u16 humidi;										// 测试变量，湿度     unsigned int 类型
int temper1;									// 测试变量，温度1    int  类型
int temper2;									// 测量变量，温度2    int  类型
float temper3;								// 测试变量，温度3    float类型
float temper4;								// 测试变量，温度4    float类型

u8 str[] = "STC32G12K128-屠龙刀三.1";	 // 字符串数组
...
// u32 对应 %lu 或 %lx，  long 对应 %ld
					printf("第%bu次发送\r\n",number1);	// 发送unsigned char类型，number1是	u8	类型，需要使用 %bu 或 %bx 
					printf("第%bd次打印\r\n",number2);	// 发送					char类型，number2是	char类型，需要使用 %bd
					printf("湿度：%hu%%\r\n",humidi);		// 发送unsigned int	类型，humidi是	u16	类型，需要使用 %hu 或 %hx
					printf("温度1：%hd℃ ",temper1);			// 发送					int	类型，temper是	int	类型，需要使用 %hd
					printf("温度2：%hd℃ ",temper2);			// 发送					int	类型，temper是	int	类型，需要使用 %hd
					printf("温度3：%4.1f℃ ",temper3);				// 发送					float	类型，temper是	int	类型，需要使用 %f，2位整数，1位小数 %4.1f
					printf("温度4：%3.1f℃\r\n",temper4);		// 发送					float	类型，temper是	int	类型，需要使用 %f，1位整数，1位小数 %3.1f
					printf("%s\r\n",str);								// 发送字符串"STC32G12K128-屠龙刀三.1"
					printf("HelloWorld!\r\n");					// 发送字符串"HelloWorld!"
				}
			}
		}
		...
		if(blinkflag == 1) 				// 当LED闪烁标志置1时
		{
			...
			// LED序号等于接收到的字符减48，比如接收到字符‘1’，LED序号=‘1’-48=1;
			Blink(LEDnums);
		}	
	}
}
