//		@布丁橘长 2023/02/12
//		定时器1-外部计数-模式0-中断法-P3.5按键每按一次，number+1，数码管显示按键次数（number值）
//		定时器1外部引脚T1-P3.5
...
void Timer1_Init(void);		//定时器0初始化函数声明
...
{
	TMOD = 0x40;			//设置定时器1-外部计数模式-模式0-16位自动重装载
	TL1 = 0xFF;				//设置定时初始值
	TH1 = 0xFF;				//设置定时初始值（计数1次就溢出，中断）
	TR1 = 1;					//定时器1开始计数
	ET1 = 1;					//使能定时器1中断
}