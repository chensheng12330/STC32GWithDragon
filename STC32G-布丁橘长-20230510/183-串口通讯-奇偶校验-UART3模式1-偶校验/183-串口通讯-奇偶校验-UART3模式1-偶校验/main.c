//	@布丁橘长 2023/04/17
//  串口通信-奇偶校验示例：偶校验-UART3-模式1，波特率115200,9位数据方式，1位起始位，1位校验位，1位停止位，
//  串口3：使用默认引脚P0.0(RxD) P0.1(TxD)
...
void Uart3Isr() interrupt 17
{
	...
	if (S3RI)										// 接收中断标志置1时
	{
		S3RI = 0;									// 清零接收中断标志
		rxdat = S3BUF;
		if(S3RB8 == Check_Even(rxdat))	// 偶校验
		{
//		buffer[wptr++] = S3BUF;	// 接收长度16
//		wptr &= 0x0f;						// 接收长度16
...
void Uart3Init()							// UART3初始化
{
	S3CON = 0xD0;								// 模式1（9位数据）、串口接收使能，禁止多机通信，定时器3做波特率发生器
	T3L = BRT;						
...
void UartSend(char dat)				// 发送字符函数
{
	while (busy);								// 当串口忙时，等待
	busy = 1;										// 忙标志置1
	S3TB8 = Check_Even(dat);		// 校验位存入S3TB8，偶校验
	S3BUF = dat;								// 发送数据dat
}
