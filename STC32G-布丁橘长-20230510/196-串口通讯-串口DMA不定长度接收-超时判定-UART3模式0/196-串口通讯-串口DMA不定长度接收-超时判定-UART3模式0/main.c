//	@布丁橘长 2023/04/26
// 	串口3DMA接收不定长度数据示例：利用串口3中断，进行超时判定，实现串口3DMA接收不定长度数据
	...
	while (1)
	{
		...
		if(B_1ms) 								//1ms 到
		{
			...
			if(RX_TimeOut > 0)			// 超时计数
			{
				...
				{
					DMA_UR3R_CR = 0x00; 					// 关闭 UART3_DMA
					printf("\r\n接收超时，以下是已接收到的数\xFD据：\r\n"); 	// UART3 发送 一个字符串,\xFD用于补全汉字‘数’的编码，防止出现乱码
					for(i=0;i<Rx_cnt;i++) printf("%bc",DMABuffer[i]);		// 将接收到的数据，发送给PC端
					printf("\r\n");								// 数据发送完成后，发送回车、换行符
					Rx_cnt = 0;										// 清零接收计数
					DMA_UR3R_CR = 0xa1; 					//bit7 1: 使能UART3_DMA，bit5 1: 开始 UART3_DMA 自动接收，bit0 1: 清除 FIFO
				}
			}
		}
	}
