//	@布丁橘长 2023/04/17
//  串口通信-奇偶校验示例：偶校验-USART1-模式3，波特率115200，9位数据方式，1位起始位，1位校验位，1位停止位
...
void Uart1Init()							// UART1初始化
{
	SCON = 0xD0;								// 模式3（9位数据）、禁止多机通信，接收使能
	T2L = BRT;						
...
void UartSend(char dat)				// 发送字符函数
{
	while (busy);								// 当串口忙时，等待
	busy = 1;										// 忙标志置1
	TB8 = Check_Even(dat);			// 校验位存入TB8，偶校验
	SBUF = dat;									// 发送数据dat
}
